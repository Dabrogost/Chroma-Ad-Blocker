...
(function() {
  ...
  const WORKER_FETCH_WRAPPER = [
    ...
    '      }',
    '    }',
    '    if(_contentSegCount===0){',
    '      out.push(\"#EXTINF:\"+targetDur+\".000,live\");',
    '      out.push(\"https://chroma-filler.invalid/\"+mediaSeq+\".ts\");',
    '    }',
    '    return out.join(\"\\n\");',
    '  }',
    ...
    '    if(typeof _url==="string" && _url.indexOf("https://chroma-filler.invalid/")===0){',
    '      return Promise.resolve(new Response(_getBlankMp4(),{status:200,headers:{"content-type":"video/mp4"}}));',
    '    }',
    ...
    '            if(!_inAdBreak){',
    '              if((Date.now()-_adEndedTs)>=_AD_END_DEBOUNCE_MS){',
    '                _inAdBreak=true;',
    '                _adElapsed=_elapsed;',
    '                _adTotal=bestTotal;',
    ...
    '          } else {',
    '            if(_inAdBreak){_inAdBreak=false;_adElapsed=0;_adTotal=0;_countedAdSegUrls.clear();_adEndedTs=Date.now();postMessage({key:\"AdBreakEnd\"});}',
