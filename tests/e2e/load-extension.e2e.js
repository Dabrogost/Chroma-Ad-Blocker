...
async function startChrome() {
  ...
  assert.ok(chromePath, 'Chrome/Chromium executable should be available; set CHROME_PATH if it is installed elsewhere');
  assert.ok(
    ...
    [
      ...
      `Install Chrome for Testing or Chromium and set CHROME_FOR_TESTING_PATH or CHROME_PATH to its chrome.exe.`
    ].join('\n')
  );
...
test('loaded extension E2E smoke', async (t) => {
  ...
  const { worker: workerTarget, sessionId: workerSession } = foundWorker;
  const extensionId = workerTarget.url.match(/^chrome-extension:\/\/([^/]+)\//)[1];

  await t.test('extension loads and service worker starts', () => {
    assert.match(extensionId, /^[a-p]{32}$/);
    assert.match(workerTarget.url, new RegExp(`^chrome-extension://${extensionId}/`));
  ...
  await t.test('popup and settings pages open', async () => {
    const popupSession = await openExtensionPage(browser.cdp, extensionId, 'ui/popup.html');
    const settingsSession = await openExtensionPage(browser.cdp, extensionId, 'ui/settings.html#proxy');

    assert.strictEqual(await evaluate(browser.cdp, popupSession, '!!document.body && location.pathname.endsWith("/ui/popup.html")'), true);
    assert.strictEqual(await evaluate(browser.cdp, settingsSession, '!!document.body && location.pathname.endsWith("/ui/settings.html")'), true);
  });

  await t.test('static rulesets are enabled and dynamic rules are installed', async () => {
    const enabledRulesets = await waitFor(async () => {
      const ids = await evaluate(browser.cdp, workerSession, 'chrome.declarativeNetRequest.getEnabledRulesets().then(ids => ids.sort())');
      return ids.length === expectedRulesets.length ? ids : null;
    }, 'enabled static rulesets');
    const dynamicRules = await waitFor(async () => {
      const rules = await evaluate(browser.cdp, workerSession, 'chrome.declarativeNetRequest.getDynamicRules()');
  ...
  await t.test('userScripts availability has an actionable diagnostic', async () => {
    const diagnostic = await evaluate(browser.cdp, workerSession, `({
